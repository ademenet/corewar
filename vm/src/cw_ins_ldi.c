...
/*   Updated: 2016/08/06 18:18:47 by ademenet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			cw_ins_ldi(t_proc *proc, t_champion *tmp, t_ocp *ocp)
{
	...
	ret = cw_get_data_ind(proc, tmp, (p[0] + p[1]) % MEM_SIZE);
	...
	mvprintw(1, 210, "%u || %u || %u || ret = %u", p[0], p[1], p[2], ret);
	mvprintw(2, 210, "%.2hhx %.2hhx %.2hhx %.2hhx", tmp->reg[p[2] - 1][0], tmp->reg[p[2] - 1][1], tmp->reg[p[2] - 1][2], tmp->reg[p[2] - 1][3]);
